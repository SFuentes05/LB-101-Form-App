...
export const getAllSolarPanels = async (req, res, next) => {
    try {
...
export const createSolarPanel = async (req, res, next) => {
    try {
        ...
        if (existingSolarPanel)
            return res.status(401).send("This company already has a solar panel form registered");
...
export const editSolarPanel = async (req, res, next) => {
    try {
        const { companyID, userID, installationLocation, agroProduction, energyConsumption, consumptionPattern, monthlyEnergyCost, primaryEnergySource, visitAvailability, contactPreference, availableSpace, ceilingType, climateCondition, electricInfrastructure, financingAvailability, ROI, subsidies, desiredInstallationTime, dueDate, comments } = req.body;
...
export const verifySolarPanel = async (req, res, next) => {
    try {
...
//# sourceMappingURL=solar-panel-controllers.js.map