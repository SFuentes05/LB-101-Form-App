...
export const createCompany = async (req, res, next) => {
    try {
        ...
        if (existingCompany)
            return res.status(401).send("This user already has a company form registered");
...
export const editCompany = async (req, res, next) => {
    try {
        ...
        if (dateCreated)
            existingCompany.dateCreated = dateCreated;
        if (name)
            existingCompany.name = name;
        if (desiredProduct)
            existingCompany.desiredProduct = desiredProduct;
        if (commercialAddress)
            existingCompany.commercialAddress = commercialAddress;
        if (plantAddress)
            existingCompany.plantAddress = plantAddress;
        if (seaLevel)
            existingCompany.seaLevel = seaLevel;
        if (taxID)
            existingCompany.taxID = taxID;
        if (webURL)
            existingCompany.webURL = webURL;
        if (processDescription)
            existingCompany.processDescription = processDescription;
        if (countriesExport)
            existingCompany.countriesExport = countriesExport;
        if (exportPercentage)
            existingCompany.exportPercentage = exportPercentage;
        if (comments)
            existingCompany.comments = comments;
